...
import {
  ...
  TooltipProvider,
  TooltipTrigger,
} from "./ui/tooltip"; // Make sure this import points to your shadcn/ui component
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  ...
  return (
    <TooltipProvider>
      <motion.tr
        ...
        className="border-b" // Use a standard border
      >
        ...
        {/* Geography Cell (IP & Country combined) */}
        {ipLookup && (
          <TableCell>
            {proxy.ip && proxy.countryCode ? (
              // Main container: Horizontal flex for [Flag] [TextStack]
              <div className="flex items-center gap-2">
                <span className="text-xl">
                  {countryCodeToFlag(proxy.countryCode)}
                </span>
                {/* Text stack container: Vertical flex for Country and IP */}
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-secondary-foreground">
                    {proxy.country ?? "Unknown"}
                  </span>

                  {/* Interactive IP row with its own Tooltip */}
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
        ...
        <TableCell className="text-right">
          {/* The ONLY button in the row is the trigger for the main dialog */}
          <AlertDialog>
            <Tooltip>
              <TooltipTrigger asChild>
            ...
            <AlertDialogContent className="min-w-[600px] bg-white/5 backdrop-blur-xl">
              ...
              <div className="mt-4 grid grid-cols-[auto_1fr] items-center gap-x-4 gap-y-3 text-sm">
                ...
                {/* Raw String with its own Copy Button */}
              ...
              <AlertDialogFooter className="mt-4">
                ...
                <Button size="sm" onClick={() => copyProxy(proxy.raw)}>
                  {isProxyCopied ? (
                    <>
                    ...
                    <>
                      <Clipboard className="mr-2 size-4" /> Copy Proxy
                    </>
                  )}
                </Button>
              </AlertDialogFooter>
